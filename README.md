...
A simple machine-learning powered classifier is provided to categorize short task descriptions.
Train the example model with:

```bash
python scripts/train_task_classifier.py
```

This generates `task_classifier.joblib`. Predict categories for new tasks with:

```bash
python scripts/predict_task_category.py "write unit tests for the new API"
```
